...
os.makedirs("../../data/TorchModelCheckpoints/AutoencoderV3", exist_ok=True)
os.makedirs("../../data/embeddings/AutoencoderV3", exist_ok=True)
os.makedirs("../../data/distances/AutoencoderV3", exist_ok=True)
...
class HypergraphAutoencoder(nn.Module):
    def __init__(self, num_node_labels, num_edge_labels, conv_dim=200, emb_dim=200):
        super(HypergraphAutoencoder, self).__init__()
        self.edge_embedding = nn.Embedding(num_edge_labels, emb_dim)
        self.encoder_conv = HypergraphConv(num_node_labels, conv_dim * 2)
        self.encoder_conv2 = HypergraphConv(conv_dim * 2, conv_dim)
    ...
    def forward(self, incidence, node_labels, hyperedge_labels):
        # One-hot encode node labels — no projection
        node_onehot = F.one_hot(
            node_labels, num_classes=self.encoder_conv.in_channels
        ).float()

        # Learnable edge embeddings
        edge_embeds = self.edge_embedding(hyperedge_labels)

        hyperedge_index = incidence.nonzero(as_tuple=False).t()

        x = self.encoder_conv(node_onehot, hyperedge_index)
        x = self.activation(x)
        x = self.encoder_conv2(x, hyperedge_index)

        node_rep = self._summarize_embeddings(x)
        edge_rep = self._summarize_embeddings(edge_embeds)
        j = torch.cat([node_rep, edge_rep], dim=0)
...
for epoch in range(numepochs):
    ...
    if epoch % 3 == 0 or epoch == numepochs - 1:
        checkpoint_path = f"../../data/TorchModelCheckpoints/AutoencoderV3/Checkpoint_Epoch_{epoch}.pth"
        checkpoint = {
            "epoch": epoch,
    ...
    with open(
        "../../data/TorchModelCheckpoints/AutoencoderV3/performances.txt", "a"
    ) as f:
        f.write(
...
checkpoint = torch.load(
    f"../../data/TorchModelCheckpoints/AutoencoderV3/Checkpoint_Epoch_{numepochs - 1}.pth"
)
...
with open("../../data/embeddings/AutoencoderV3/feature_matrix.pkl", "wb") as f:
    pkl.dump(feature_matrix, f)
with open("../../data/embeddings/AutoencoderV3/orglabels.pkl", "wb") as f:
...
with open("../../data/distances/distance_matrix.pkl", "wb") as f:
    pkl.dump(distance_matrix, f)
with open("../../data/distances/ORG_distance_matrix.pkl", "wb") as f:
    pkl.dump(orglabels, f)
