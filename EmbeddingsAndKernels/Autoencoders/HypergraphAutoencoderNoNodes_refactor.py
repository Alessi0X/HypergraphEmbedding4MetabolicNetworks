...
class HypergraphAutoencoder(nn.Module):
    ...
    def forward(self, incidence, node_labels, hyperedge_labels):
        ...
        node_onehot = F.one_hot(node_labels, num_classes=self.encoder_conv.in_channels).float()
...
numepochs = 13
os.makedirs("data/TorchModelCheckpoints/AutoencoderV5", exist_ok=True)
...
for epoch in range(numepochs):
    ...
    if epoch % 3 == 0:
        checkpoint_path = f"data/TorchModelCheckpoints/AutoencoderV5/Checkpoint_Epoch_{epoch}.pth"
        checkpoint = {
            'epoch': epoch,
    ...
    with open("data/TorchModelCheckpoints/AutoencoderV5/performances.txt", "a") as f:
...
checkpoint = torch.load("data/TorchModelCheckpoints/AutoencoderV5/Checkpoint_Epoch_12.pth")
...
#save feature mat
with open("data/TorchModelCheckpoints/AutoencoderV5/feature_matrix.pkl", "wb") as f:
    pkl.dump(feature_matrix, f)

#save organism labels
orglabels = [graph["ID"].upper() for graph in DATASET]

with open("data/TorchModelCheckpoints/AutoencoderV5/orglabels.pkl", "wb") as f:
...
with open("data/distances/AutoencoderV5NoNodes.pkl", "wb") as f:
    pkl.dump(distance_matrix, f)

with open("data/distances/ORG_AutoencoderV5NoNodes.pkl", "wb") as f:
    pkl.dump(orglabels, f)
