...
os.makedirs("../../data/TorchModelCheckpoints/AutoencoderV1", exist_ok=True)
os.makedirs("../../data/embeddings/AutoencoderV1", exist_ok=True)
os.makedirs("../../data/distances/AutoencoderV1", exist_ok=True)
...
def process_hypergraph(
    ...
    """
    Process a hypergraph by creating an incidence matrix and extracting node/edge labels.
    This function takes a collection of hyperedges and creates a local incidence matrix
    representation along with corresponding node and hyperedge labels based on global
    mappings.
    Args:
        hyperedges (list): A list of hyperedges, where each hyperedge is a collection
                          of nodes that belong to that hyperedge.
        global_mapping_nodes (dict): A dictionary mapping node identifiers to their
                                   global label indices.
        global_mapping_edges (dict): A dictionary mapping edge tuples (sorted) to their
                                   global label indices.
        dense (bool, optional): If True, returns a dense tensor for the incidence matrix.
                               If False, returns a sparse COO tensor. Defaults to True.
    Returns:
        tuple: A tuple containing:
            - incidence (torch.Tensor): The incidence matrix of shape (num_nodes, num_hyperedges)
              where incidence[i, j] = 1 if node i belongs to hyperedge j, 0 otherwise.
            - node_labels (torch.Tensor): A 1D tensor of node labels corresponding to the
              local node ordering.
            - hyperedge_labels (torch.Tensor): A 1D tensor of hyperedge labels corresponding
              to the hyperedge ordering.
    """
...
class HypergraphAutoencoder(nn.Module):
    def __init__(self, num_node_labels, num_edge_labels, embed_dim, conv_dim):
    ...
    def forward(self, incidence, node_labels, hyperedge_labels):
        ...
        hyperedge_index = incidence.nonzero(as_tuple=False).t()  # convert to COO tensor

        # Node embeddings after convolution
        x = self.encoder_conv(node_embeds, hyperedge_index)

        # Average node embedding
        avg_node_embedding = x.mean(dim=0)  # shape: [conv_dim]
        ...
        avg_edge_embedding = edge_embeds.mean(dim=0)  # shape: [embed_dim]

        # Create combined vector j
        j = torch.cat(
            [avg_node_embedding, avg_edge_embedding], dim=0
        )  # shape: [conv_dim + embed_dim]

        # Reconstruction logits and sigmoid
        recon_logits = torch.matmul(x, edge_embeds.t())
...
for epoch in range(numepochs):
    ...
    for incidence, node_labels, hyperedge_labels in tqdm(
        ...
        optimizer.zero_grad()
        recon_logit, j = model(incidence, node_labels, hyperedge_labels)
    ...
    if epoch % 3 == 0 or epoch == numepochs - 1:
        checkpoint_path = f"../../data/TorchModelCheckpoints/AutoencoderV1/Checkpoint_Epoch_{epoch}.pth"
        checkpoint = {
            "epoch": epoch,
    ...
    with open(
        "../../data/TorchModelCheckpoints/AutoencoderV1/performances.txt", "a"
    ) as f:
        f.write(
...
# Housekeeping
del recon, recon_logit, hyperedges

# ========================
# Extracting the Global Latent Features
# ========================
...
checkpoint = torch.load(
    f"../../data/TorchModelCheckpoints/AutoencoderV1/Checkpoint_Epoch_{numepochs - 1}.pth"
)
...
with torch.no_grad():
    for incidence, node_labels, hyperedge_labels in tqdm(hypergraph_dataset):
        ...
        node_labels = node_labels.to(device)
        hyperedge_labels = hyperedge_labels.to(device)
        _, j = model(incidence, node_labels, hyperedge_labels)
        # recon_cpu = _.cpu().numpy()
        all_j.append(j.cpu().numpy())

# Convert list of arrays to a 2D numpy array
feature_matrix = np.stack(all_j)
...
with open("../../data/embeddings/AutoencoderV1/feature_matrix.pkl", "wb") as f:
    pkl.dump(feature_matrix, f)
with open("../../data/embeddings/AutoencoderV1/orglabels.pkl", "wb") as f:
    pkl.dump(orglabels, f)

# Housekeeping
del all_j, incidence, node_labels, hyperedge_labels, j, hypergraph_dataset
...
with open("../../data/distances/AutoencoderV1/distance_matrix.pkl", "wb") as f:
    pkl.dump(distance_matrix, f)
with open("../../data/distances/AutoencoderV1/ORG_distance_matrix.pkl", "wb") as f:
    pkl.dump(orglabels, f)
