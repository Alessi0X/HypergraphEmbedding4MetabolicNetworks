...
class HypergraphAutoencoder(nn.Module):
    ...
    def forward(self, incidence, node_labels, hyperedge_labels):
        ...
        j = torch.cat([avg_node_embedding, avg_edge_embedding], dim=0)  # shape: [conv_dim + embed_dim]
...
numepochs = 13
os.makedirs("data/TorchModelCheckpoints/Autoencoder", exist_ok=True)
...
for epoch in range(numepochs):
    ...
    if epoch % 3 == 0:
        checkpoint_path = f"data/TorchModelCheckpoints/Autoencoder/Checkpoint_Epoch_{epoch}.pth"
        checkpoint = {
            'epoch': epoch,
    ...
    with open("data/TorchModelCheckpoints/Autoencoder/performances.txt", "a") as f:
...
model = HypergraphAutoencoder(num_node_labels=len(global_node_to_idx),
                              num_edge_labels=len(global_edge_to_idx),
                              embed_dim=200,
                              conv_dim=200).to(device)
optimizer = optim.Adam(model.parameters(), lr=1e-3)

checkpoint = torch.load("data/TorchModelCheckpoints/Autoencoder/Checkpoint_Epoch_12.pth")
...
#save feature mat
with open("data/TorchModelCheckpoints/Autoencoder/feature_matrix.pkl", "wb") as f:
    pkl.dump(feature_matrix, f)

#save organism labels
orglabels = [graph["ID"].upper() for graph in DATASET]

with open("data/TorchModelCheckpoints/Autoencoder/orglabels.pkl", "wb") as f:
...
with open("data/distances/AutoEncoderV1.pkl", "wb") as f:
    pkl.dump(distance_matrix, f)
with open("data/distances/ORG_AutoEncoderV1.pkl", "wb") as f:
    pkl.dump(orglabels, f)
