...
prepgraphlist = []
for graph in tqdm(graphlist, desc="Preparing graphs for Graph2Vec (C3)"):
...
# Filtering for C3: keep only labels with minocc >= 100
clev = "C3"
...
print("Running for class level: C3")
print(
    f"for C3 there are {len(filtered_graphs)} eligible graphs, {len(temporgs)} eligible organisms and {templabels[clev].nunique()} unique classes"
)

# Load Optuna study and get the best parameters
with open("../../data/optuna_studies/graph2vecC3.pkl", "rb") as f:
    tempstudy = pickle.load(f)

tempparams = tempstudy.best_params
print(f"Best parameters for C3 are: {tempparams}")

# Start timer
start = time.time()
print("Computing Graph2Vec (C3)")
...
print(f"Embedding for class C3 shape is: {tempemb.shape}")

embtosave = {"classlev": "C3", "embedding": tempemb, "orglist": temporgs}

with open("../../data/embeddings/Graph2Vec/C3_embedding.pkl", "wb") as f:
